...
def port_invest(count, start_date, alpha_pos, port_value):
    if len(alpha_pos) == 0:
        return port_value
    ...
    start_date_actual = closest_date(start_month)
    end_date_actual = closest_date(end_month)
    
    # Handle case where start and end dates are the same
    if start_date_actual == end_date_actual:
        available_dates = sorted([d for d in data["IWM"].index if d > start_date_actual])
        if available_dates:
            end_date_actual = available_dates[0]
        else:
            return port_value
    
    ETF_ratios = []
    for ticker in alpha_pos:
        ticker_start_price = data[ticker].loc[start_date_actual, "Adj Close"]
        ticker_end_price = data[ticker].loc[end_date_actual, "Adj Close"]
        ETF_ratios.append(ticker_end_price / ticker_start_price)
    
    return port_value / len(alpha_pos) * sum(ETF_ratios)
    
    ETF_ratios = []
    for ticker in alpha_pos:
        ticker_start_price = data[ticker].loc[start_date_actual, "Adj Close"]
        ticker_end_price = data[ticker].loc[end_date_actual, "Adj Close"]
        ratio = ticker_end_price/ticker_start_price
        ETF_ratios.append(ratio)
        print(f"{ticker}: {ticker_start_price:.2f} -> {ticker_end_price:.2f} (ratio: {ratio:.4f})")
    
    new_port_value = port_value / len(alpha_pos) * sum(ETF_ratios)
    print(f"Portfolio: {port_value:.2f} -> {new_port_value:.2f}")
...
for i in range(60):
    ...
    # Last row accounting
        ...
        # Investing portfolio
        positive_alpha_ETFs = [k for k, v in ETF_dict.items() if v > 0]
        port_value = port_invest(i, start_date, positive_alpha_ETFs, port_value)
